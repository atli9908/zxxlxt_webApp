...
<script>
import {getPublishers} from '../../utils/api/test';
export default {
  data() {
    return {
  ...
  methods: {  //返回
    backRouter(){   
      this.$router.push('/psych');
    },
    submitBtn(){  //确定
      this.$router.push({name:'Psych',query:{name:this.value,publisherUid:this.publisherUid,starttime:this.startTime,endtime:this.endTime}});
    },
    startOnConfirm(date) {
